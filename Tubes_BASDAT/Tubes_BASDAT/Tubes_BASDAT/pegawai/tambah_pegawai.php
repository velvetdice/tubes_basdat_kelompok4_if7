            ...
            <div>
              <form class="form-horizontal" action="proses_tambah_pegawai.php" method="POST">
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="nip" id="nip" placeholder="Masukkan NIP untuk Pegawai baru" required />
                  </div>
                </div>
                <div class="form-group">
                  <label for="nama_pegawai" class="col-sm-2 control-label">Nama Pegawai</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="nama_pegawai" id="nama_pegawai" placeholder="Masukkan Nama untuk Pegawai baru" required />
                  </div>
                </div>
                <div class="form-group">
                    ...
                    <select name="id_jabatan" id="disabledSelect" class="form-control">
                      ...
                      if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                          echo '<option value="' . $row["id_jabatan"] . '">' . ucfirst($row["id_jabatan"]) . '</option>';
                        }
                      }
                      ?>
                    </select>
                  ...
                  <div class="col-sm-10">
                    <select name="id_tunjangan" id="disabledSelect" class="form-control">
                      ...
                      if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                          echo '<option value="' . $row["id_tunjangan"] . '">' . ucfirst($row["id_tunjangan"]) . '</option>';
                        }
                      }
                      ?>
                    </select>
